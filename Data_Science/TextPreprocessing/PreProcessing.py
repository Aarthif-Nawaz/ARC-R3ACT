...
def pre_process_review(review, type):
    ...
    if type=="svr":
        # pre-processing required by the SVR/MLP
        pre_processed_review=pre_processing_labelled_data(review)
    elif type=="cluster":
        # preprocessing required by the MLP model used to cluster the reviews
        pre_processed_review=reg_preprocessing(review)
        pre_processed_review=pre_processing_labelled_data(pre_processed_review)
    elif type=="lexicon":
        # pre_processing required for the lexicon sentiment analysis
        pre_processed_review = de_emojize(review)
        pre_processed_review=reg_preprocessing(pre_processed_review)
        # preprocessing that is performed before feature extraction is performed
    elif type=="fe":
        pre_processed_review=preprocessing_fe(review)
