...
class TestPreprocess(unittest.TestCase):
    ...
    def test_pre_process_review(self):
        ...
        self.assertEqual(Data_Science.PreProcess.PreProcess.pre_process_review("No pasting. Why is such a basic feature missing? This completely ruined my first impression of the app, uninstalled.", "cluster"), "no paste basic feature miss completely ruin impression app uninstalle", "Successfully tested pre processed review for cluster")
        self.assertEqual(Data_Science.PreProcess.PreProcess.pre_process_review("No pasting. Why is such a basic feature missing? This completely ruined my first impression of the app, uninstalled.", "lexicon"), "No pasting basic feature missing ? completely ruined impression app uninstalled ", "Successfully tested pre processed review for lexicon")
    ...
    def test_expand_contractions(self):
        self.assertEqual(Data_Science.PreProcess.PreProcess.expand_contractions("i can't read the apps settings"), "i can not read the apps settings","Successfully expanded contractions")
    #Perform unit testing to check if reviews are pre processed according to SVR/MLP
    def test_reg_preprocessing(self):
        self.assertEqual(Data_Science.PreProcess.PreProcess.reg_preprocessing("Very elegant and user friendly interface. Truly recommended, thanks for the amazing experience, devs :)"), "Very elegant user friendly interface Truly recommended thanks amazing experience devs :) ", "Successfully pre processed review for SVR/MLP Models")
    # Perform unit testing to check if reviews are pre processed according to FeatureExtraction
    def test_preprocessing_fe(self):
        self.assertEqual(Data_Science.PreProcess.PreProcess.preprocessing_fe("Very elegant and user friendly interface. Truly recommended, thanks for the amazing experience, devs :)"),"elegant user friendly interface . Truly recommended , thanks amazing experience , devs :) ", "Successfully pre processed review for feature extraction")
