...
def lemmatization(text):
    ...
    count = 0
    for word in text:
        pos = get_wordnet_pos(tagged[count][1])
        if (pos == ""):
            lem = lemmatizer.lemmatize(word)
            lemmatized = lemmatized + lem + " "
        else:
            lem = lemmatizer.lemmatize(word,pos)
            lemmatized = lemmatized + lem + " "
        count = count + 1
    return lemmatized
...
def text_preprocessing(notProcessedText):
    ...
    if notProcessedText is not None:
        ...
        notProcessedText = lemmatization(notProcessedText)
